...
public class Trace {
  ...
  private String[] excludes = {"java.*", "javax.*", "sun.*",
      "com.sun.*"};
  private String[] includes = {"com.google.wireless.android.finsky.*"};
   ...
   * Generate the trace.
   */
  Trace(String[] args) throws Exception {
    ...
    for (inx = 0; inx < args.length; ++inx) {
      ...
      if (arg.equals("-output")) {
        ...
        port = Integer.parseInt(args[++inx]);
      } else if (arg.equals("-class")) {
        classToTrack = args[++inx];
      } else if (arg.equals("-start")) {
        startLine = Integer.parseInt(args[++inx]);
      } else if (arg.equals("-end")) {
        endLine = Integer.parseInt(args[++inx]);
      } else if (arg.equals("-include")) {
        includes = args[++inx].split(",");
  ...
  private VirtualMachine attachTarget() throws IOException, IllegalConnectorArgumentsException {
    ...
    Map<String, Argument> args = chosenConnector.defaultArguments();
    args.get("port").setValue(Integer.toString(port));
    return chosenConnector.attach(args);
  }
   ...
   */
  private void generateTrace(PrintWriter writer) {
    vm.setDebugTraceMode(debugTraceMode);
    EventThread eventThread = new EventThread(vm, includes, classToTrack, startLine, endLine,
        writer);
   ...
   */
  Map<String, Connector.Argument> connectorArguments(LaunchingConnector connector,
      String mainArgs) {
